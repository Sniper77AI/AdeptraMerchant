 ...
 * the content_rewrite generator, and the mcp_scaffold generator. Future
 * artifact types (jsonld, llms_txt, robots_patch) are sibling modules added
 * here, not folded into this file — and since every generator takes the same
 * ArtifactContext, adding one never requires changing runArtifacts()'s
 * signature again.
 *
 ...
 * generator — the existing pure/sync generators (manifest, feed,
 * mcp_scaffold) need no changes; a non-Promise return value awaits to itself.
 */
...
export type { ArtifactType, ArtifactChangelog, ArtifactDraft, ArtifactContext, ArtifactFile, ArtifactFileTree } from "./types.ts";
